...
static bool heap_check(const uint64_t a, const uint64_t b) {
    cmb_assert(cmi_event_queue != NULL);
...
static void heap_grow(void) {
    cmb_assert(cmi_event_queue != NULL);
    cmb_assert(heap_size < (UINT32_MAX - heap_chunk));
...
void cmb_event_schedule(cmb_event_func *action, void *subject, void *object, double rel_time, int16_t priority) {
    cmb_assert(rel_time >= 0.0);
    cmb_assert(heap_count <= heap_size);
    cmb_assert(cmi_event_queue != NULL);
...
void cmb_event_reschedule(const uint64_t index, const double time) {
    cmb_assert(index <= heap_count);
    cmb_assert(time >= cmi_event_sim_time);
