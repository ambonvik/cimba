...
simulated terrain. The terrain model is a synthetic 1000 x 1000 nautical mile patch generated by Perlin
noise with 1 arcsecond (~30 m) resolution. It is divided in three
...
shoot-and-scoot pattern. After firing, it selects a random direction, speed, and
distance, and drives there before hiding stationary at the new location. Target
coordinates are recalculated on each sensor update. The target
...
* Doppler processing: no MTI / pulse-Doppler clutter cancellation or STAP.
  Clutter is suppressed by amplitude CFAR alone. This is a significant departure from
  model realism, since it lets our model radar detect small stationary targets in heavy
  clutter, where it in reality would not be able to.
...
leaves a sufficiently detailed radar model to give reasonable and compute-intensive
...
with unrelated code. Similarily, there is a separate ``cmb_process`` that only updates
...
The central characteristic of this version of the model is that it is
